...
import { Settings, Moon, Sun, Monitor, Check, Globe, Languages, LogOut, User, LayoutDashboard, LogIn } from "lucide-react";
...
export function SettingsButton() {
  ...
  const themes = [
    { name: "light", icon: Sun, label: t("settings.theme.light", { defaultValue: "Light" }) },
    { name: "dark", icon: Moon, label: t("settings.theme.dark", { defaultValue: "Dark" }) },
    { name: "system", icon: Monitor, label: t("settings.theme.system", { defaultValue: "System" }) },
  ];
      ...
      <DialogContent className="sm:max-w-[400px] bg-white/95 dark:bg-[#1A1A1A]/95 backdrop-blur-xl border border-[#D4AF37]/30 dark:border-[#3D3D3D] rounded-2xl p-0 overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-8">
              ...
              {themes.map((tMode) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-medium tracking-wider uppercase">{tMode.label}</span>
                  </button>
                );
              })}
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-light tracking-wide">{language.label}</span>
                    </div>
          ...
          {isAdmin && (
            <div className="space-y-4">
              <h3 className="text-xs font-medium uppercase tracking-[0.2em] text-[#8B7355] dark:text-amber-200/50 flex items-center gap-2">
                {t("settings.administration", { defaultValue: "Administration" })}
              </h3>
              <Link href="/admin/products" className="no-underline" onClick={() => setOpen(false)}>
                <div className="w-full flex items-center gap-3 p-4 rounded-xl border border-[#D4AF37]/30 bg-[#D4AF37]/5 dark:bg-[#D4AF37]/10 text-[#B8A072] dark:text-[#E5C96B] hover:bg-[#D4AF37]/15 transition-all duration-300">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-medium tracking-wide">
                      {t("settings.adminDashboard", { defaultValue: "Admin Dashboard" })}
                    </span>
                    <span className="text-[10px] opacity-70 uppercase tracking-tighter">
                      {t("settings.adminSubtitle", { defaultValue: "Manage products, users & more" })}
                    </span>
                  </div>
                </div>
              </Link>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate text-gray-900 dark:text-gray-100">{user?.fullName || user?.email}</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span className="text-xs">{t("settings.logout", { defaultValue: "Logout" })}</span>
                  </Button>
                </div>
              </div>
            ) : (
              <Link href="/login" className="no-underline" onClick={() => setOpen(false)}>
                <Button className="w-full justify-center gap-2 h-12 rounded-xl bg-[#D4AF37] hover:bg-[#C19B2B] text-white shadow-lg shadow-[#D4AF37]/20 border-none">
                  ...
                  <span className="text-sm font-medium uppercase tracking-widest">{t("settings.signIn", { defaultValue: "Sign In" })}</span>
                </Button>
              </Link>
            )}
          </div>
